...
const POSES: Record<string, Pose> = {
  ...
  sujud:   { rootY: 0.34, rootZ: -0.08, torsoX: 96 * D, headX: 36 * D, headY: 0, shoulderX: 100 * D, elbowX: 12 * D, hipX: 35 * D, kneeX: -125 * D, shoulderZ: 14 * D },
...
function buildRig(canvas: HTMLCanvasElement, reduced: boolean): Rig | null {
  ...
  arch.add(c1, c2, ring, lamp, lampLight);
...
export function Salah3D({ step, playing }: SceneProps) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (rig.idle && !reduced) rig.azimuth += dt * 0.07;
